...
emitOverviewBatch : (Buffer -> IO ()) -> String -> Nat -> List TodoEvent -> IO ()
emitOverviewBatch _ _ _ [] = pure ()
emitOverviewBatch emit streamId startVersion (event :: rest) = do
  let nextVersion = S startVersion
  emit (frameOverviewEvent streamId nextVersion event)
  emitOverviewBatch emit streamId nextVersion rest

registerCleanupLocal : IORef.IORef ClientUnsubs -> String -> IO () -> IO ()
registerCleanupLocal ref key cleanup = do
...
overviewCleanupKey : String -> String
overviewCleanupKey clientId = "overview::" ++ clientId
...
subscribeOverview env unsubsRef clientId =
  MkPublisher $ \subscriber => do
    ...
    unsub <- runReaderT env $
      subscribeCategory {m=ReaderT (AppEnv TodoEvent) IO} {stream=String} {ev=TodoEvent} isTodoStream $ \streamId, startVersion, events =>
        liftIO (emitOverviewBatch subscriber.onNext streamId startVersion events)
    let cleanup = runReaderT env unsub
    registerCleanupLocal unsubsRef (overviewCleanupKey clientId) cleanup
    pure ()
