...
public export
data Command = Create String | Increment | Decrement | Reset

public export
data Rejection = AlreadyCreated | NotCreatedYet | AtMinimum

public export
data CounterEvent = Created String | Incremented | Decremented | ResetToZero
...
public export
data Action = CreateCounterAction | IncrementCounterAction | DecrementCounterAction | TapSignalAction | LowerSignalAction | DanceSignalAction

public export
data Intent = PromptCreate | RunIncrement | RunDecrement | SendTapSignal | SendLowerSignal | SendDanceSignal

public export
record ExternalSignal where
  constructor MkExternalSignal
  rawAction : String

public export
data SignalRejection = UnknownExternalSignal String
...
public export
renderSignalRejection : SignalRejection -> String
renderSignalRejection (UnknownExternalSignal raw) = "unknown external signal: " ++ raw
...
renderAction TapSignalAction = "External signal: tap"
renderAction LowerSignalAction = "External signal: lower"
renderAction DanceSignalAction = "External signal: dance"
...
data CounterLegal : Command -> CounterState -> Type where
  ...
  CanReset : CounterLegal Reset (MkCounterState (Just title) value)
...
resetLegal : (state : CounterState) -> Either Rejection (CounterLegal Reset state)
resetLegal (MkCounterState Nothing _) = Left NotCreatedYet
resetLegal (MkCounterState (Just _) _) = Right CanReset
...
implementation Projection CounterEvent CounterState where
  ...
  evolve state ResetToZero = MkCounterState (name state) 0
...
implementation Decider List Command Rejection CounterEvent CounterState where
  ...
  legal Reset state = resetLegal state
  ...
  decide Reset _ CanReset = [ResetToZero]
...
implementation Projection CounterEvent CounterView where
  ...
  evolve view Incremented = let next = S (value view) in MkCounterView (label view) next (romanDigit next) True
  evolve view Decremented = let next = decrementNat (value view) in MkCounterView (label view) next (romanDigit next) True
  evolve view ResetToZero = MkCounterView (label view) 0 "" (created view)
...
implementation ScreenActions Screen CounterBundle Action Intent where
  ...
  intentForAction TapSignalAction = SendTapSignal
  intentForAction LowerSignalAction = SendLowerSignal
  intentForAction DanceSignalAction = SendDanceSignal

  availableScreenActions bundle CounterHome =
    if created (view bundle)
      then [IncrementCounterAction, DecrementCounterAction, TapSignalAction, LowerSignalAction, DanceSignalAction]
      else [CreateCounterAction]

public export
implementation Translation ExternalSignal Command SignalRejection where
  translate signal =
    case rawAction signal of
      "tap" => Right Increment
      "lower" => Right Decrement
      other => Left (UnknownExternalSignal other)

threshold : Nat
threshold = 3

public export
automationStep : AutomationStep CounterView CounterEvent Command
automationStep = MkAutomationStep evolveView decideCommands
  where
    evolveView : CounterView -> CounterEvent -> CounterView
    evolveView = evolve

    decideCommands : CounterView -> CounterEvent -> List Command
    decideCommands view Incremented =
      if created view && value view == threshold
        then [Reset]
        else []
    decideCommands _ _ = []
